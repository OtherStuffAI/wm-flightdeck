...
`http://<transport.fips.npub>.fips:<port>` and native pairing uses that transport
npub. Generation time is signature-bound and valid for five minutes, with a
...
`connect({ endpoint, serviceNpub: transportNpub })`, verifies the returned
