...
describe('flight deck summary template', () => {
  ...
  it('keeps in-view channel tabs alongside the new expanded sidebar hierarchy', () => {
    ...
    expect(html).toMatch(/class="global-pg-channel-bar"[\s\S]*?:class="\{ 'global-pg-channel-bar-sidebar-expanded': !\$store\.chat\.navCollapsed \}"[\s\S]*?x-show="\$store\.chat\.isLoggedIn"/s);
