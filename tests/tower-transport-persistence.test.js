...
const bridgeFetch = vi.fn(async () => Response.json({ service_npub: serviceNpub }));
bridge.connect.mockImplementation(async input => ({ version: 2, ...input, fetch: bridgeFetch, disconnect: vi.fn() }));
beforeEach(async () => {
  await db.tower_transport_preferences.clear();
  await initializeTowerTransports({ transport: null });
});
...
it('retains endpoint and identity through FIPS, HTTPS, database reopen, settings and FIPS reconnect', async () => {
  ...
  await initializeTowerTransports({ transport: bridge });
  ...
  await initializeTowerTransports({ transport: bridge });
  expect(bridge.connect).toHaveBeenLastCalledWith({ endpoint, peerNpub, purpose: 'tower' });
...
it('keeps the legacy source on migration write failure and retries successfully', async () => {
  ...
  await expect(initializeTowerTransports({ transport: null })).rejects.toThrow('disk full');
...
it.each(['{broken', JSON.stringify({ [tower]: { ...preference, endpoint: 'https://wrong.example' } })])(
  'fails initialization closed and retains malformed legacy data for recovery: %s', async raw => {
    ...
    await expect(initializeTowerTransports({ transport: bridge })).rejects.toThrow();
    expect(localStorage.getItem(key)).toBe(raw);
