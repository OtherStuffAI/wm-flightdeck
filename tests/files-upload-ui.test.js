...
describe('files upload UI', () => {
  ...
  it('renders shared folders inside Files and removes standalone Drive navigation', () => {
    ...
    expect(html).toContain("filesControlsOpen: false");
    expect(html).toContain("filesActionsOpen: false");
    ...
    expect(html).toContain('aria-label="Search and filter files"');
    expect(html).toContain('class="files-mobile-icon-btn files-mobile-add-btn"');
    expect(html).toContain('class="doc-actions-popover files-mobile-action-popover"');
    expect(html).toContain("'files-toolbar-mobile-open': filesControlsOpen");
    ...
    expect(css).toMatch(/\.files-toolbar\s*\{[\s\S]*display:\s*none;/);
    expect(css).toMatch(/\.files-toolbar-mobile-open\s*\{[\s\S]*display:\s*grid;/);
    expect(css).toMatch(/\.files-pane-heading\s*\{[\s\S]*display:\s*none;/);
    ...
    expect(css).toContain('calc(100dvh - 8rem)');
